...
const main = async () => {
    ...
    let selectYears = years.filter((it) => parseInt(it) <= 2017);
    let selectData = salesData.filter((it) => it.year.getFullYear() <= 2017);
    ...
    console.log(stackData.map((it) => it.key))
    ...
    column.addEventListener('scroll', (e) => {
        ...
        switch (pageNum) {
            ...
            case 2:
                ...
                break;
            case 3:
                const other90 = d3.scaleLinear().domain([0, window.innerHeight]).range([ xScale(yearParser('1982')), xScale(yearParser('1995'))]);
                ...
                break;
            case 4:
                const trans9010 = d3.scaleLinear().domain([0, window.innerHeight]).range([ 1995, 2005]);
                ...
                break;
            case 5:
                    const scale1 = d3.scaleLinear().domain([0, window.innerHeight]).range([ xScale(yearParser('1995')), xScale(yearParser('2005'))]);
